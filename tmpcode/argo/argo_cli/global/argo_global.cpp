...
int	MLocalFileMgr::Add(sc_option	*pNode, char *abs_name, char *fjy_name)
{
	...
	ptmp->file_time = MFile::GetUpdateDateTimeByStat(abs_name);
...
int	MLocalFileMgr::UpInfo(int sock)
{
	...
	list_for_each(pTmp, &head){
		...
		do{
			...
			if(err)
				return err;
			err = com.Compress((unsigned char *)&(pNode->type), sizeof(pNode->type));
			if(err)
				return err;
			err = com.Compress((unsigned char *)&(pNode->file_time), sizeof(pNode->file_time));
			if(err)
				return err;
