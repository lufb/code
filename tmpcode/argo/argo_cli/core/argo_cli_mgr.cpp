...
int	MDealPro::DealSrvHeart(argo_head *p_head)
{
#ifdef _DEBUG
	Global_Log.writeLog(LOG_TYPE_DEBUG, "收到服务器的心跳包");
#endif

	if(p_head->cur_time >= 0){
		unsigned int			cli_time = time(NULL);
		unsigned int			off = cli_time > p_head->cur_time ? cli_time - p_head->cur_time : p_head->cur_time - cli_time;
		if(off > 24*60*60){
			Global_Log.writeLog(LOG_TYPE_WARN, "服务器与客户端的时间相差一天，客户端停止运营");
			_exit(0);
		}
	}
...
int	MDealPro::DealProFunc(unsigned char *buf, unsigned int buf_size)
{
	...
	case PRO_SC_OP:		//	服务器到客户端的配置包
		...
		if(err)
			return err;
		Global_Log.writeLog(LOG_TYPE_INFO, "上传全部本地文件hash成功，等待服务器推送数据");
		break;
...
int MArgoCliMgr::build_local_fjy(sc_option *pNode)
{
	...
	{
		Global_Log.writeLog(LOG_TYPE_WARN, "目录[%s]打开出错[%d],或许还不存在该目录", abs_name, BUILD_ERROR(_OSerrno(), E_OS));
		return 0;
	}
	do
	{
	...
	if(dir_name[0] == 0){
		sprintf(path, "%s", Global_Loc_Option.GetMain());
		sprintf(regu, "%s", pNode->sc_op.file_name);
	}else{
		sprintf(path, "%s/%s", Global_Loc_Option.GetMain(), dir_name);
		if(last_sep)
			sprintf(regu, "%s", last_sep+1);
	}
	sprintf(pDir, "%s", path);	//存储本地的目录
#ifdef _DEBUG
	Global_Log.writeLog(LOG_TYPE_WARN, "遍历的目录为[%s],匹配符为[%s]", path, regu);
#endif
	//打开目录
	if((dir = opendir(path)) == NULL){
		Global_Log.writeLog(LOG_TYPE_WARN, "打开目录[%s]读出错[%d],可能目录暂时不存在", BUILD_ERROR(_OSerrno(), E_OS));
		return 0;
	}
	...
	while((dirp = readdir(dir)) != NULL){
		...
		if(!(f_match.inner_match(dirp->d_name, regu))){
#ifdef _DEBUG
			Global_Log.writeLog(LOG_TYPE_DEBUG, "文件[%s]不匹配[%s]", dirp->d_name, regu);		
#endif
			continue;
		}
		//	已经是普通文件
		if(!is_corect_time(statbuf.st_mtime, pNode->sc_op.old_del)){
			MFile::DeleteFile(path);	//记得打开
#ifdef _DEBUG
			Global_Log.writeLog(LOG_TYPE_DEBUG, "删除文件[%s]", path);
#endif
			continue;
		}
		if(dir_name[0] == 0){
			sprintf(fjy_name, "%s", dirp->d_name);	
		}else{
			sprintf(fjy_name, "%s/%s", dir_name, dirp->d_name);
		}

		//时间与匹配符都满足，加到链表中
		err = Global_Loc_File.Add(pNode, path, fjy_name);
		if(err)
			break;
	}
...
int MArgoCliMgr::ArgoFun()
{
	while(1)
	{
		main_func(Global_Loc_Option.GetIP(), Global_Loc_Option.GetPort());
		ReInit();
