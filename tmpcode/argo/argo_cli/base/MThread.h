...
//------------------------------------------------------------------------------------------------------------------------------
//下面定义了线程函数
typedef void * (__stdcall * tagMThreadFunction)(void *);
/************************************************************************
 *	功能：定义打印输出报告回调函数
...
protected:
	...
	__int64						m_uiLoopCount;	//线程循环次数
	__int64						m_uiWorkCount;	//工作次数
	__int64						m_uiRealWorkUSECCount;	//线程实际工作时间统计(微秒)
...
public:
	// 启动、停止线程
	int  StartThread(MString strName, tagMThreadFunction fpFunction,void * lpParam,bool bCheckAlive = true);
...
public:
	...
	// add by yuanjj for cereating thread 2014-09-30
	static int create_thread( size_t stack_size, tagMThreadFunction fpFunction, void* lpParam, eng_tid_t& tid );
