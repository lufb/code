...
int  MMD5::MD5(const char * lpInBuf,int iInSize,char szOutBuf[16])
{
	...
	if ( iextendsize > 9000 )
	{
		lpextendbuf = new char[iextendsize];
		if ( lpextendbuf == NULL )
		{
			//没有足够的内存可以分配
    ...
    memcpy(lpextendbuf,lpInBuf,iInSize);

    for ( i=iInSize;i<iextendsize-8;i++ )
    {
        if ( i == iInSize )  lpextendbuf[i] = (unsigned char)128;
        else    lpextendbuf[i] = 0;
    }
    itemp = iInSize * 8;
    memcpy((char *)&lpextendbuf[iextendsize-8],(char *)&itemp,4);
    memset((char *)&lpextendbuf[iextendsize-4],0x00,4);

    for (i=0;i<iextendsize/64;i++ )
    {
        a = aa;
        b = bb;
        c = cc;
