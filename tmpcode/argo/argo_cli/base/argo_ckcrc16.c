...
static const char rcsid[] =
	"@(#) $Id: argo_ckcrc16.c,"
	"v 1.00 2016/01/13 09:00:00 CST yangling Exp $ (LBL)";
#endif
...
{
	size_t			i;
	const char *	vptr = (const char*)buff;

	for (i = 0; i < size; i++)
	{
		crc16 = crc16_table[((crc16 >> 8) & 0xff) ^ (vptr[i] & 0xff)] ^ (crc16 << 8);
	}
...
{
	unsigned short	cksum = argo_ckcrc16_init();

	cksum = argo_ckcrc16_update(cksum, buff, size);

	return argo_ckcrc16_final(cksum);
}
