...
//modify by liuqy 20110225 打印超时时间
...
//		my_snprintf(tempbuf, 256, "|  线程ID        线程名称         开始时间     状态  循环次数  工作次数 利用率    统计时长     工作时长  利用率");
		...
		my_snprintf(tempbuf, 256, "|  线程ID        线程名称         开始时间       状态   次数率 时间率 附加");
...
//		if(!poThreadBase->m_blBeginWorkFlag || 0 == poThreadBase->m_ulThreadID)
		...
		if(0 != poThreadBase->m_uiLoopCount)
		{
			...
			my_snprintf(tempbuf,256,"| %8u  %-18s %-15s %-8s %6.2f %6.2f %4d", 
				poThreadBase->m_ulThreadID, poThreadBase->m_strObjectName, szDateTime, szWorkState,
				 dCountRate,
				 dTimeRate, poThreadBase->m_lAddtionData);
			/*
		...
		{
			//add by liuqy 20100926 for 打印线程的附加数据
			my_snprintf(tempbuf,256,"| %8u  %-18s %-15s %-8s %6s %6s %4d", 
				poThreadBase->m_ulThreadID, poThreadBase->m_strObjectName, szDateTime, szWorkState,
					"未统计", 		"未统计", poThreadBase->m_lAddtionData);
			/*
...
//	printf("______________________Thread(%08X)(%ld) already exit\n", m_hRecordData, lWaitTime);fflush(stdout);
			...
			{
#ifdef _DEBUG
	//printf("______________________Thread(%08X)(%ld) Timeout\n", m_hRecordData, lWaitTime);fflush(stdout);
#endif
				//	超时了
...
// add by yuanjj 2014-09-30
/**
 * cereating thread ( static function )
 * 
 * @stack_size			[in] the stack size, (default 0)
 * @fpFunction			[in] the thread function
 * @lpParam				[in] the thread function param
 * @tid					[out] the thread id
 *
 * return		
 * 		==	0		successful
 * 		!=	0		errorcode	
 */
int MThread::create_thread( size_t stack_size, tagMThreadFunction fpFunction, void* lpParam, eng_tid_t& tid )
{	
#ifndef LINUXCODE
...
/**
 *create_thread  detach_thread exit_thread 请注意配合使用
 *
 *	main:
 *		create_thread();
 *
 ...
 *		exit_thread();
 */
void MThread::detach_thread(void)
{
#ifndef LINUXCODE
#else
