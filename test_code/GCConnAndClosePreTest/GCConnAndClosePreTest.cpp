...
int fillParam(PARAM &param, int argc, char *argv[])
{
	...
	strncpy(param.fileName, argv[8], sizeof(param.fileName));

	MAX_FRAME_SIZE = param.maxFrameSize;//
	strncpy(TEST_DATA, param.fileName, sizeof(TEST_DATA));
...
SOCKET doGcSocket(PARAM &param, cls_proxy_green& gree)
{
	...
	err = proxy_green("127.0.0.1", err, param.DstIP, param.DstPort, &greeSocket);
	if(err != 0)
	{
		SET_RED
