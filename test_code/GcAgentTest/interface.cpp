...
int __stdcall proxy_green_new(	IN char* gcIP, IN unsigned short green_port,
						  IN tagNewQLGCProxy_Certify &pro, 
							OUT int* psockfd )
{
	int		res;
	...
	tagNewQLGCProxy_Certify*	gc_certify	= (tagNewQLGCProxy_Certify*)(buf + sizeof(qlgc_apply));
	
	memcpy( gc_apply->szFlag, "QLGC", strlen("QLGC") );
	gc_apply->ucApplyType	= 1;
	gc_apply->usSize		= sizeof(tagNewQLGCProxy_Certify);

	memcpy(gc_certify, &pro, sizeof(tagNewQLGCProxy_Certify));
	len = sizeof(qlgc_apply) + sizeof(tagNewQLGCProxy_Certify);
	
	res = ::send(*psockfd, buf, len, 0);
	if( res == SOCKET_ERROR )
		return GetLastError();

	res = ::recv(*psockfd, buf, len, 0);
	if( res == SOCKET_ERROR )
		return GetLastError();

	if(res == sizeof(qlgc_apply)+sizeof(qlgc_crtret))
		return 0;//成功，就不校验数据内容了
