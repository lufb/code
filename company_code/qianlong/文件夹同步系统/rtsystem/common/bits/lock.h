...
/**
 *	futex_mutex_init - 初始化一个互斥锁
 *
 *	return:
 *		0		成功
 *		!0		失败
 */
static INLINE_FN int futex_mutex_init(struct futex_mutex *mutex)
{
#ifdef WIN32
	InitializeCriticalSection(&mutex->cs);
	return 0;
#else
	return pthread_mutex_init(&mutex->__lock, NULL);
#endif
}

/**
 *	futex_mutex_destroy - 销毁一个互斥锁
 *
 *	return:
 *		0		成功
 *		!0		失败
 */
static INLINE_FN int futex_mutex_destroy(struct futex_mutex *mutex)
{
#ifdef WIN32
	DeleteCriticalSection(&mutex->cs);
	return 0;
#else
	return pthread_mutex_destroy(&mutex->__lock);
#endif
}
 ...
 *		!0		失败
 */
static INLINE_FN int futex_mutex_lock(struct futex_mutex *mutex)
{
#ifdef WIN32
	EnterCriticalSection(&mutex->cs);
	return 0;
#else
	return pthread_mutex_lock(&mutex->__lock);
#endif
}

/**
 *	futex_mutex_unlock - 解锁
 *
 *	return:
 *		0		成功
 *		!0		失败
 */
static INLINE_FN int futex_mutex_unlock(struct futex_mutex *mutex)
{
#ifdef WIN32
	LeaveCriticalSection(&mutex->cs);
	return 0;
#else
	return pthread_mutex_unlock(&mutex->__lock);
#endif
}
