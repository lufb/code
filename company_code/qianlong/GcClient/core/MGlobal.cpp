...
/** Global var define*/
unsigned long				Global_DllVersion = (((unsigned long)200 << 16) + 0);	/** Version*/
...
int  GcCEndWork()
{
	...
	if(Global_IsRun == false)
		Global_Log.writeLog(LOG_TYPE_INFO, "GcClient还没运行，直接返回");
	else
		Global_IsRun = false;

	/* 主要功能模块释放 */
 	Global_CorMgr.waitExit(rc, LEVEL_COUNT);
 	Global_Log.writeLog(LOG_TYPE_INFO, "GcClient功能模块已完成退出");

	Global_Option.destroy();
	Global_Log.writeLog(LOG_TYPE_INFO, "GcClient配置模块已完成退出");
	...
	Global_Log.writeLog(LOG_TYPE_INFO, 
		"GcClient已完全退出，每层退出码分别为[%d][%d][%d][%d][%d]", 
