...
unsigned int __stdcall MTrans::transThread(void * in)
{
	...
	MLOCALTRANS				localTrans;				/* 局部变量,这个局部变量稍微有点大，应该在创建线程时，把栈设大些*/
	fd_set					fdRead;
	...
	do{
		...
		if(!list_empty(&localTrans.m_stUsed))
		{
			/* 有需要检测的套接字 */
			FD_ZERO(&fdRead);
			/* 遍历本地链表，添加select元素 */
			list_for_each(pTmp, &localTrans.m_stUsed)
			{
				pNode = list_entry(pTmp, TRANS_SOCKET_PEER, m_lListNode);
			...
			if(rc > 0){ /* 检测到套接字 */
				list_for_each_safe(pTmp, pSafe, &localTrans.m_stUsed)
				{
 					pNode = list_entry(pTmp, TRANS_SOCKET_PEER, m_lListNode);
					if(FD_ISSET(pNode->m_cli.m_sock, &fdRead))	/* CLI -> SRV */
						if(pObj->_trans(localTrans, pNode, false) != 0)
							continue;	/* 提前终止 */
					if(FD_ISSET(pNode->m_srv.m_sock, &fdRead))
						pObj->_trans(localTrans, pNode, true); /* SRV -> CLI */
				}
			}else{
				if(rc < 0){ /* 发生错误 */
					Global_Log.writeLog(LOG_TYPE_ERROR,
...
int MTrans::_trans(MLOCALTRANS &local, TRANS_SOCKET_PEER *pNode, bool isSrv)
{
	...
	if(rc != 0)
	{
		...
		/* 重新初始化该节点 */
		local.initTransNode(pNode);
		/* 资源回归 */
		MList::_del2insert(&(pNode->m_lListNode), &local.m_stFree);
	}
...
/**
 *	_insertToLocal		-		加到本地trans链表中
 *
 *	@local			[in/out]	本地trans结构
 *	@cliSock:		[in]		客户端套接字
 *	@srvSock:		[in]		服务端套接字
 *
 *
 ...
 *		!=0						失败
 */
int	MTrans::_insertToLocal(MLOCALTRANS &local,SOCKET cliSock, SOCKET srvSock)
{
	if(list_empty(&local.m_stFree))	/** list is empty*/
		return BUILD_ERROR(0, ESOCKOVERFLOW);
	
	TRANS_SOCKET_PEER	*pNode = list_entry(local.m_stFree.next, TRANS_SOCKET_PEER, m_lListNode);
