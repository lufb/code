...
struct buddy2
{
    unsigned size;			/*	管理内存的总大小	*/
    unsigned longest[1];	/*	1表示可以向后扩展	*/
};
...
void	pr_self(struct buddy2	*self)
{
#ifdef TEST_LUFB
	int				i;
...
struct buddy2* buddy2_new( int size )
{
	...
	node_size = size * 2;	/* 满二叉时分配的大小为其2倍	*/
    self = (struct buddy2*)ALLOC( node_size * sizeof(unsigned));
    self->size = size;
