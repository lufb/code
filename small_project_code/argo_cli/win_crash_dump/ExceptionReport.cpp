...
bool ExceptionReport::WriteMiniDump(PEXCEPTION_POINTERS pExceptionInfo)
{
	...
	pMiniDumpWriteDump(
		...
		GetCurrentProcessId(),
		m_hDumpFile,
		// modify by yjj 20121220，增加写入dump的选项，生成的*.dmp文件中包含更多的信息，但这样会导致dmp文件增大（增大几十倍以上），
		// 明天就是传说中的世界末日，但愿这个改动带来一个愉快的末日
		// MiniDumpNormal,
		//(MINIDUMP_TYPE)(MiniDumpWithDataSegs|MiniDumpWithProcessThreadData|MiniDumpWithHandleData|MiniDumpWithUnloadedModules),
