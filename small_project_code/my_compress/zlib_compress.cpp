...
int
my_compress_final(compress_ctx *ctx, unsigned char *g_buf)
{
	int				ret;
...
_my_compress(compress_ctx	*ctx,
			 unsigned int	avail_in, 	unsigned char *next_in, unsigned char *g_buf)
{
	int			ret;
...
my_compress(compress_ctx	*ctx,
			unsigned char	*in_buf, 	size_t in_size, unsigned char *g_buf)
{
	unsigned int	i = 0;
	...
	for(i = 0; i < blocks; ++i){
		ret = _my_compress(ctx, CHUNK, in_buf + i * CHUNK, g_buf);
		if(ret )
			return ret;
	}
	if(last){
		ret = _my_compress(ctx, last, in_buf + blocks * CHUNK, g_buf);
		if(ret )
			return ret;
	}
